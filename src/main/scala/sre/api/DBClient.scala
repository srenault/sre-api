...
case class DBClient[F[_]](transactionsDir: java.io.File)(implicit connection: Connection, F: Effect[F]) {
  ...
  def selectPeriodIndexes(maybeBeforePeriod: Option[YearMonth], maybeAfterPeriod: Option[YearMonth], limit: Int, yearMonthOrdering: Ordering.Value = Ordering.DESC): F[List[CompletePeriodIndex]] =
    F.delay {
      try {
        val parser = CompletePeriodIndex.parser(transactionsDir)
    ...
    F.delay {
      try {
        SQL("SELECT * FROM FINANCE_PERIODINDEX WHERE yearmonth = {yearmonth}")
          ...
          .as(CompletePeriodIndex.parser(transactionsDir).singleOpt)
      } catch {
        case e: Exception =>
          e.printStackTrace
          throw e
...
object DBClient {
  ...
  def stream[F[_]: Effect](settings: Settings): Stream[F, DBClient[F]] = {
    ...
    val dbClient = DBClient(settings.finance.transactionsDir)
