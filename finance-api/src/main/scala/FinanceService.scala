...
case class FinanceService[F[_] : Logger](
      ...
      case Some(lastPeriod) =>
        dbClient.selectStatements(maybeAfterDate = Some(lastPeriod.endDate)).flatMap { allStatements =>
          analyticsClient.computeCurrentPeriod(allStatements) match {
            ...
            case None =>
              Logger[F].warn(s"No period were found from statements").map(_ => None)
          }
        }
      case None =>
  ...
  def getAccountState(accountId: String, maybePeriodDate: Option[YearMonth]): F[Option[(Period, CMAccountState)]] = {
val maybeAccountSettings = settings.cm.accounts.find(_.id == accountId)
    ...
    (maybeAccountSettings, maybePeriodDate) match {
      ...
      case (_, Some(periodDate)) =>
        analyticsClient.getAccountStateForPeriod(accountId, periodDate).value.flatMap {
          ...
          case None =>
            Logger[F].warn(s"Not able to get account state for period ${periodDate}").map(_ => None)
        }
          ...
          case Some(lastPeriod) =>
            dbClient.selectStatementsByAccountId(accountId, maybeAfterDate = Some(lastPeriod.endDate)).flatMap { statements =>
              analyticsClient.computeCurrentPeriod(statements) match {
                ...
                case None =>
                  Logger[F].warn("No period were found from statements").map(_ => None)
              }
            }
  ...
  def getPeriods(maybeBeforePeriod: Option[YearMonth], maybeAfterPeriod: Option[YearMonth]): F[(List[Period], Boolean, Boolean)] = {
    for {
  ...
  def getStatementsForPeriod(periodDate: YearMonth): F[Option[(Period, List[CMStatement])]] = {
