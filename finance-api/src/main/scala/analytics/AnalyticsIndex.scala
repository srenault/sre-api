...
case class AnalyticsIndexClient[F[_]](
  ...
  def computePeriodIndexesFrom(
      statements: List[CMStatement]
  ): List[PeriodIndex] = {
    val segments = AnalyticsIndexClient.computeSegmentIndexesStep(
      statements,
      partitions = Nil
    )(isWageStatement)
    val (periods, _) = AnalyticsIndexClient.computePeriodIndexesStep(
      segments,
      periods = Nil,
      pendingSegments = Nil
    )
  ...
  def computeLatestPeriodIndexes(nmonths: Int = 4): F[List[PeriodIndex]] =
    settings.accountsDir match {
      case accountDirs @ (accountDir :: _) =>
        val ofxFiles =
          OfxDir.listFiles(accountDir).sortBy(-_.date.toEpochDay).take(nmonths)
  ...
  private def computePeriodIndexes(
      accountDirs: List[File],
      ofxFiles: List[OfxFile],
      seed: Option[PeriodIndex] = None
  ): F[List[PeriodIndex]] = {
    AnalyticsIndexClient
      .computePeriodIndexes(accountDirs, ofxFiles, seed)(isWageStatement)
...
object AnalyticsIndexClient {
  ...
  private def computePeriodIndexesStep(
      segments: List[SegmentIndex],
      periods: List[PeriodIndex],
      pendingSegments: List[SegmentIndex]
  ): (List[PeriodIndex], List[SegmentIndex]) = {

    @annotation.tailrec
    def step(
        sortedSegments: List[SegmentIndex],
        accPeriods: List[PeriodIndex],
        pendingSegments: List[SegmentIndex],
        lastIncludedStatements: List[CMStatement]
    ): (List[PeriodIndex], List[SegmentIndex]) = {
      ...
      sortedSegments match {
        case (segment @ SegmentIndex(
          ...
          val allStatements = CMStatement
            .merge(segmentsForPeriod.flatMap(_.statements.toList))
            .sorted(CMStatement.ORDER_DESC)

          val statementsForPeriod = allStatements.takeWhile(
            _.date.isAfter(wageStatement.date.minusDays(1))
          )
          ...
          if (segmentsForPeriod.nonEmpty) {
            ...
            val (updatedAccPeriods, updatedLastIncludedStatements) =
              maybeLastPeriod match {
                  ...
                  if (
                    scala.math.abs(
                      ChronoUnit.DAYS.between(
                        wageStatement.date,
                        lastPeriod.wageStatements.last.date
                      )
                    ) <= 10
                  ) { // Include new wage statement
                    val newStatements = statementsForPeriod.filterNot(s1 =>
                      lastIncludedStatements.exists(s2 => s1.id == s2.id)
                    )
                    val updatedLastPeriod = lastPeriod.includeNewWageStatement(
                      wageStatement,
                      newStatements,
                      partitions
                    )
                    ...
                    val (statementsForLastPeriod, statementsForCurrentPeriod) =
                      statementsForPeriod.span(
                        _.date.isEqual(lastPeriod.startDate)
                      )
    ...
    step(
      ...
      accPeriods = periods,
      pendingSegments,
      lastIncludedStatements = Nil
    )
  }

  private def computeSegmentIndexesStep(
      statements: List[CMStatement],
      partitions: List[OfxFile]
  )(isWageStatement: CMStatement => Boolean): List[SegmentIndex] = {

    @annotation.tailrec
    def step(
        statements: List[CMStatement],
        acc: List[SegmentIndex]
    ): List[SegmentIndex] = {
      statements match {
        case Nil => acc
  ...
  private def fillMissingBalancesByAccount(
      periodIndexes: List[PeriodIndex]
  ): List[PeriodIndex] = {
    periodIndexes.foldLeft(List.empty[PeriodIndex]) { case (acc, periodIndex) =>
      acc.lastOption match {
        case Some(previousPeriodIndex) =>
          val updatedBalancesByAccount =
            previousPeriodIndex.balancesByAccount ++ periodIndex.balancesByAccount
          val updatedPeriodIndex: PeriodIndex = periodIndex match {
            ...
            case incompletePeriod: IncompletePeriodIndex =>
              incompletePeriod.copy(balancesByAccount =
                updatedBalancesByAccount
              )
          }
  ...
  def computePeriodIndexes[F[_]](
      accountDirs: List[File],
      ofxFiles: List[OfxFile],
      seed: Option[PeriodIndex] = None
  )(
      isWageStatement: CMStatement => Boolean
  )(implicit F: Sync[F]): F[List[PeriodIndex]] = {

    def step(
        accountDirs: List[File],
        sortedOfxFiles: List[OfxFile],
        accSegments: List[SegmentIndex],
        accPeriods: List[PeriodIndex]
    ): F[List[PeriodIndex]] = {
      sortedOfxFiles match {
        case ofxFile :: remainingOfxFiles =>
          val partitions: List[OfxFile] = accountDirs
            .map { accountDir =>
          ...
          partitions
            .map(p => OfxStmTrn.load(p).map(p -> _))
            .sequence
            .map {
            ...
            .flatMap { statements =>
              val sortedStatements =
                CMStatement.merge(statements).sorted(CMStatement.ORDER_DESC)
                ...
                case Some(lastPeriod) =>
                  sortedStatements
                    .dropWhile { st =>
                      ...
                      (st.date.isEqual(
                        lastPeriod.startWageStatement.date
                      ) && !lastPeriod.isWageStatement(st))
                    }
              ...
              val segments =
                computeSegmentIndexesStep(nextStatements, partitions)(
                  isWageStatement
                )

              val (updatedAccPeriods, updatedAccSegments) =
                computePeriodIndexesStep(segments, accPeriods, accSegments)
        ...
        case Nil =>
          val periods = accPeriods.headOption match {
            case Some(
                  lastPeriod
                ) => // Complete the last period with remaining statements if needed
              val remainingSegments = accSegments.takeWhile(segment =>
                segment.statements.exists(_.date == lastPeriod.startDate)
              )
              val remainingStatements = CMStatement
                .merge(remainingSegments.flatMap(_.statements).toList)
                .sorted(CMStatement.ORDER_DESC)
              val statementsForLastPeriod = remainingStatements.takeWhile(
                _.date.isEqual(lastPeriod.startDate)
              )
              val partitions = remainingSegments.flatMap(_.partitions)
              val updatedLastPeriod = lastPeriod.includeStatements(
                statementsForLastPeriod,
                partitions
              )
