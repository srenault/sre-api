...
case class FinanceHttpService[F[_]: Logger](
    ...
    extends FinanceHttpServiceDsl[F] {
  ...
  val routes: HttpRoutes[F] =
    HttpRoutes.of[F] {
      ...
      case GET -> Root / "finance" / "accounts" =>
        service.getAccountsOverview().flatMap {
          case Some(accountsOverview) =>
            val json =
              FinanceHttpService.accountsOverviewEncoder(accountsOverview)
            Ok(json)
          case None =>
            NotFound()
        }

      case GET -> Root / "accounts" / AccountIdVar(
            accountId
          ) :? OptionalPeriodDateQueryParamMatcher(maybeValidatedPeriod) =>
        WithPeriodDate(maybeValidatedPeriod) { maybePeriodDate =>
          service.getAccountState(accountId, maybePeriodDate).flatMap {
            case Some(periodWithAccountState) =>
              val json =
                FinanceHttpService.accountStateEncoder(periodWithAccountState)
              Ok(json)
      ...
      case GET -> Root / "finance" / "analytics" :? OptionalBeforePeriodDateQueryParamMatcher(
            maybeValidatedBeforePeriod
          ) +& OptionalAfterPeriodDateQueryParamMatcher(
            maybeValidatedAfterPeriod
          ) =>
        WithPeriodDate(maybeValidatedBeforePeriod) { maybeBeforePeriod =>
          WithPeriodDate(maybeValidatedAfterPeriod) { maybeAfterPeriod =>
            service.getPeriods(maybeBeforePeriod, maybeAfterPeriod).flatMap {
              periodsWithPagination =>
                val (periods, hasPreviousPage, hasNextPage) =
                  periodsWithPagination
                Ok(
      ...
      case GET -> Root / "finance" / "analytics" / "period" / PeriodDateVar(
            periodDate
          ) =>
...
object FinanceHttpService {
  ...
  val accountsOverviewEncoder: Encoder[CMAccountsOverview] =
    new Encoder[CMAccountsOverview] {
      final def apply(accountsOverview: CMAccountsOverview): Json = {
        accountsOverview.asJson(CMAccountsOverview.encoder)
      }
    }

  val accountStateEncoder: Encoder[(Period, CMAccountState)] =
    new Encoder[(Period, CMAccountState)] {
      final def apply(
          periodWithAccountState: (Period, CMAccountState)
      ): Json = {
  ...
  val periodsEncoder: Encoder[(List[Period], Boolean, Boolean)] =
    new Encoder[(List[Period], Boolean, Boolean)] {
      final def apply(
          periodsWithPagination: (List[Period], Boolean, Boolean)
      ): Json = {
  ...
  val statementsEncoder: Encoder[(Period, List[CMStatement])] =
    new Encoder[(Period, List[CMStatement])] {
      final def apply(
          periodWithStatements: (Period, List[CMStatement])
      ): Json = {
