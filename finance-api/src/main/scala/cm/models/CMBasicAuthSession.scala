...
object CMBasicAuthSession {
  ...
  def create(idSesResponseCookie: ResponseCookie, otherResponseCookies: List[ResponseCookie]): CMBasicAuthSession = {
    val idSesCookie = RequestCookie(idSesResponseCookie.name, idSesResponseCookie.content)
    val otherCookies = otherResponseCookies.map(cookie => RequestCookie(cookie.name, cookie.content))
