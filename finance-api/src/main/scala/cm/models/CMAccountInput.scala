...
object CMAccountInput {
  ...
  ): Either[String, List[CMAccountInput]] = {
    doc
      .select("#account-table label")
      .asScala
      .zipWithIndex
      .map { case (domLabel, index) =>
  ...
  def parseOrFail(doc: org.jsoup.nodes.Document): List[CMAccountInput] =
    parse(doc) match {
      case Left(error) =>
        sys.error(s"Unable to parse $doc as CMAccountInput: $error")
